...
// simulator around in a circle with a constant  angle and velocity on a
// flat terrain.
//
...
int latency = static_cast<int>(0.1 / dt + 0.5);

//Increasiung smoothness
const double coeff_derivative_delta = 700; 
const double coeff_derivative_a = 1.;     
const double coeff_penalize_delta = 200.; 
const double coeff_penalize_a = 1.;      
...
class FG_eval 
{
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N; t++) {
        fg[0] += coeff_cte * CppAD::pow(vars[cte_start + t] - ref_cte, 2);
        fg[0] += coeff_epsi * CppAD::pow(vars[epsi_start + t] - ref_epsi, 2);
    ...
    for (int t = 1; t < N; t++) {
      ...
      //p'(x) = coeffs[1] + 2*coeffs[2] * x  + 3*coeffs[3] * x^2
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  steering_offset = solution.x[delta_start + latency];
  throttle = solution.x[a_start + latency];
